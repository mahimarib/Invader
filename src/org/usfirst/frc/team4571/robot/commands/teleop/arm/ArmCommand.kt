...
object ArmCommand : Command() {
    ...
    override fun execute() {
        ...
        when {
            rightTrigger > 0 && rightTrigger > leftTrigger -> ArmSystem.setArmMotors(-rightTrigger)
            leftTrigger > 0 && leftTrigger > rightTrigger -> ArmSystem.setArmMotors(leftTrigger)
            Robot.GAMEPAD.buttonB.get() -> ArmSystem.setArmMotors(OUTTAKE_SPEED)
            Robot.GAMEPAD.buttonA.get() -> ArmSystem.setArmMotors(LOWER_SPEED)
            else -> ArmSystem.stop()
        }
    }
